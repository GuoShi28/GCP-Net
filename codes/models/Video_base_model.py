...
class VideoBaseModel(BaseModel):
    ...
    def optimize_parameters_rgb(self, step):
        ...
        self.fake_H = self.netG(self.var_L, self.var_NM)

        # pdb.set_trace()
        fake_H_rgb = process.process_train(self.fake_H, self.red_gains, \
            self.blue_gains, self.cam2rgbs)
        real_H_rgb = process.process_train(self.real_H, self.red_gains, \
            self.blue_gains, self.cam2rgbs)

        l_pix = self.cri_pix(self.fake_H, self.real_H) + self.l_pix_w * self.cri_pix(fake_H_rgb, real_H_rgb)
        l_pix.backward()
